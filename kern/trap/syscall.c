...
void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	...
	if (virtual_address == 0 || (virtual_address >= USER_HEAP_MAX) || (virtual_address < USER_HEAP_START) || (virtual_address + size >= USER_HEAP_MAX) || (virtual_address + size < USER_HEAP_START))
	{
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	{
		uint32 *pgdir = (uint32 *)a1;
		uint32 va = (uint32)a2;
		uint32 **user_ptr = (uint32 **)a3;
		uint32 *kernel_pt = NULL;
		int res = get_page_table(pgdir, va, &kernel_pt);
		if (user_ptr)
		{
			if (res == TABLE_NOT_EXIST || kernel_pt == 0)
				*user_ptr = 0;
			else
				/* Provide user a read-only view of the page table via UVPT */
				*user_ptr = (uint32 *)(UVPT + PDX(va) * PAGE_SIZE);
		}
		return 0;
	}
	break;
