...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//Your code is here
		    ...
		    for (uint32 i = start_va ; i < (uint32)start_va+size ; i += PAGE_SIZE)
		    {
//		    	acquire_kspinlock(klock);
		       ...
		       //create table
		        ...
		        // marking
		        pt_set_page_permissions(e->env_page_directory, i,PERM_UHPAGE,1);
...
//		acquire_kspinlock(klock);
		uint32* p=NULL;
			...
			if(ret==TABLE_IN_MEMORY){
				...
				pt_set_page_permissions(e->env_page_directory,(uint32)i,0,PERM_UHPAGE);
