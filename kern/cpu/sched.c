...
struct Env* fos_scheduler_PRIRR()
{
	...
	if (curr_env != NULL && curr_env->env_status == ENV_READY) {
		sched_insert_ready(curr_env);
//		enqueue(&ProcessQueues.env_ready_queues[curr_env->priority], curr_env);
	}


	uint8 chosen_q = -1; // 34an a3rf bs el mo4kla feen ya abdo, momkn t4elo 3ady
	for(uint8 i = 0; i < num_of_ready_queues; i++) {
		if (!LIST_EMPTY(&(ProcessQueues.env_ready_queues[i]))) {
		    ...
		    chosen_q = i;
		    //cprintf("PICK env %d from queue %d\n", next_env->env_id, chosen_q);
		    break;
		}
	}

	if (next_env != NULL && chosen_q >= 0) {
		kclock_set_quantum(quantums[chosen_q]); // not needed but leh la2
	}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	    {
			// 0 % tresh is always zero so make sure ticks are above zero (mn el a5r eta2kd en fe time 3da mn el start)
	        if (threshold > 0 && ticks > 0 && (ticks % threshold) == 0) {

	            acquire_kspinlock(&(ProcessQueues.qlock));

	            for (int lvl = 1; lvl < num_of_ready_queues; lvl++) {
	                struct Env *e;
	                // for (int i =0; i < queue_size(&ProcessQueues.env_ready_queues[i]); i++)
	                while ((e = dequeue(&(ProcessQueues.env_ready_queues[lvl]))) != NULL) {
	                    if (e->priority > 0)
	                        e->priority--;

	                    enqueue(&(ProcessQueues.env_ready_queues[e->priority]), e);
	                }
	            }
...
void update_WS_time_stamps()
{
    ...
    // If no environment is running --- nothing to update
    ...
    LIST_FOREACH(wse, &(env->page_WS_list))
    {
        ...
        uint32 *ptr_pt = NULL;
        get_page_table(env->env_page_directory, va, &ptr_pt);

        if (ptr_pt == NULL)
            continue;

        uint32 pte = ptr_pt[PTX(va)];
        ...
        if (pte & PERM_USED)
        {
            ...
            ptr_pt[PTX(va)] &= ~PERM_USED;
            tlb_invalidate(env->env_page_directory, (void *)va);
        }
    }
