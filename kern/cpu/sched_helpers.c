...
void env_set_priority(int envID, int priority)
{
	...
	//panic("env_set_priority() is not implemented yet...!!");
	    ...
	    if (priority < 0 || priority >= num_of_ready_queues) {
	        cprintf("env_set_priority: invalid priority %d", priority);
	        return;
	    }
	    ...
	    e = find_env_in_queue(&ProcessQueues.env_new_queue, envID);

	    if (e == NULL) {
	    	for(int i = 0; i < num_of_ready_queues; i++) {
	    		e = find_env_in_queue(&ProcessQueues.env_ready_queues[i], envID);
	    		if (e != NULL) {break;}
	    	}
	    }
	    ...
	    if (e == NULL || e->env_status == ENV_FREE) { // 3malha just in case, bs kda kda fault handler is hanlding those
	        cprintf("env_set_priority: env %d not found\n", envID);
	        release_kspinlock(&ProcessQueues.qlock);
	        return;
	    }

	    uint8 new_pr = (uint8)priority;

	    if (e->env_status == ENV_READY) { //if ready update its place in ready queue
	        sched_remove_ready(e);

	        e->priority = new_pr;

	        sched_insert_ready(e);
	    } else {						// if anything else just change priority
	        e->priority = new_pr;
	    }

	    //cprintf("the env status is: %d\n", e->env_status);
	    //cprintf("setPri: env %d -> priority %d\n", envID, priority);
